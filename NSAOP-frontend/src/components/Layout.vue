...
import {SET_ROLE, SET_TOKEN, SET_USERNAME} from "@/common/store";
import {revokeToken} from "@/common/auth";
import {TIMEOUT, CONNECTION_LOST} from "@/common/utils";
  ...
  mounted() {
    ...
    if (this.$store.getters.token !== undefined && this.$store.getters.token != ' ') {
      authTest().then(res => {
        ...
        this.$store.commit(SET_ROLE, res.role)
      }).catch(err => {
        // console.log(err);
        if (err === TIMEOUT || err === CONNECTION_LOST) {
          this.$message({
            message: "服务器正忙，请重试",
            type: "error",
            duration: 1000,
